 ...
 *	в прерывании приема;
 *	- при обнаружении ошибки приема необходимо вызвать функцию setReadState();
 ...
 *	readData().
 *
 *	Байты данных на передачу возвращает функция pull(), при этом необходимо
 *	каждый раз проверять их наличие isSendData().
 *
...
private:
	...
	/// Максимальное количество регистров доступных для чтения в одном запросе.
	static const uint16_t MAX_NUM_REGISTERS = 4;
...
public:
	...
	typedef enum {
		...
		CHECK_ERR_LEN			///< Ошибка: неверная длина сообщения.
	} CHECK_ERR;
...
private:
	...
	bool checkAddressLan(uint8_t adr) const;
