...
Светодиоды:
- VD19 - показывает активность протокола общения с МК БСП. Горит если есть связь.
- VD20 - показывает активность протокола Modbus. Горит если есть связь. 

В случае отсутствия принятых сообщений в течении 50 мс, полученные данные с 
соответствующего протокола обнуляются. Например, если пропала связь с МК БСП, 
в Modbus будут уходить значения всех регистров 0x0000. И наоборот.
...
//---Стандартный----------------------------------------------------------------
    ...
    Команда 0x11 - передается в БСП
        55 AA 11 05 01 02 03 04 05 25
                 |  |     |     |  | КС, младший байт суммы байт начиная с команды
                 |  |     |     | контрольные частоты принятые по Modbus
                 |  |     | входы 32..17 принятые по Modbus, 0x0403 ()
                 |  | входы 16..1 принятые по Modbus, 0x0201
                 | количество байт данных, всегда 5.

    Команда 0x12 - принимается из БСП
        55 AA 12 05 01 02 03 04 05 26
                 |  |     |     |  | КС, младший байт суммы байт начиная с команды
                 |  |     |     | контрольные частоты для передачи в Modbus
                 |  |     | выходы 32..17 для передачи в Modbus, 0x0403 ()
                 |  | выходы 16..1 для передачи в Modbus, 0x0201
                 | количество байт данных, всегда 5.      
...
//---MODBUS---------------------------------------------------------------------
    ...
    Holding Registers (0x03)
        ...
        1 или 3 -> входы с 16 по 1.
        2 или 4 -> входы с 32 по 17.
...
//---MODBUS ПРИМЕР КОМАНД ДЛЯ РАБОТЫ--------------------------------------------        
    ...
    Нужно проверить описание содержимого команд, по-моему есть ошибки!
    CRC передается младшим байтом вперед, все остальное - старшим.
...
Установка и сброс значений дискретных входов (DI):
    ...
    Запрос  : 01 06 00 02 02 01 E8 AA
                    ...
                    | адрес регистра, в данном случае для DI с 16 по 1
